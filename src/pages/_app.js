...
import {MY_PREFERRED_COLORS, MY_PREFERRED_THEME} from '../constants/names';
...
function MyApp({Component, pageProps}) {
  ...
  const changeColors = colorObj => {
    ...
    setItemInLS(MY_PREFERRED_COLORS, JSON.stringify({...customColors, ...colorObj}));
  };
  ...
  useEffect(() => {
    ...
    const MY_COLORS = getItemFromLS(MY_PREFERRED_COLORS);

    if (MY_COLORS) {
      setCustomColors(MY_COLORS);
    } else {
      setItemInLS(MY_PREFERRED_COLORS, JSON.stringify(customColors));
    }
    if (default_theme) {
  ...
  return (
    ...
    >
      <QueryClientProvider client={queryClient}>
        <Head>
          <title>Adozillion | Homes & Reality</title>
          <meta
            name="description"
            content="Adozillion is a a construction company with a clear vision to create contemporary and urban living for discerning individuals within small communities / Estates."
          />
